...
export const DestinatariosContainer = ({ idUsuario, tipoUsuario, tipoPublicacion }) => {
	...
	const descripcionMateria = useSelector((state) => state.publicaciones.descripcionMateria);
	...
	const destinatarios = useSelector((state) => state.publicaciones.destinatarios);
	...
	const { query: queryAlumno, vars: varsAlumno, metodo: metodoAlumno } = getQueryAlumnosBy({
		ciclos,
		niveles,
		modalidades,
		grados,
	...
	const { query: queryParentesco, vars: varsParentesco, metodo: metodoParentesco } = getQueryParentescosBy({
		idCiclo,
		idNivel,
		idModalidad,
		idGrado,
	...
	const [
		...
		{ loading: loadingParentesco, error: errorParentesco, data: dataParentesco }
	...
	const { query: queryEnrollment, vars: varsEnrollment, metodo: metodoEnrollment } = getQueryEnrollmentsBy({
		idCiclo,
		idNivel,
		idModalidad,
		idGrado,
	...
	const [
		...
		{ loading: loadingEnrollment, error: errorEnrollment, data: dataEnrollment }
	...
	useEffect(
		() => {
			if (tipoDestinatario === 'ALUMNOS') {
				...
				if (dataAlumno && dataAlumno[metodoAlumno]) {
					listaAlumnos = dataAlumno[metodoAlumno].map((p) => {
						return { id: p.idAlumno, nombreCompleto: p.nombreCompleto, tipo: 'ALUMNO' };
					});
			...
			if (tipoDestinatario === 'ALUMNOS & FAMILIARES') {
				...
				alumnoFamilia.forEach((af) => {
					listaConcatDestinatarios = [...listaConcatDestinatarios, af, ...af.familiares];
				});

				listaConcatDestinatarios = [...listaConcatDestinatarios, ...listaAcademicos]
		...
		[dataAlumno, dataParentesco, dataProfesor, dataUsuario, parentescos, dataEnrollment]
	);
	...
	const handleChangeMateria = (e) => {
		if (e.target.value !== '') {
			...
			if (pos > 0) {
				dispatch(setDescripcionMateria(e.nativeEvent.target[index].text.substring(pos + 1)));
			} else {
				...
				<Grid container spacing={3}>
					<Grid item xs={12} sm={12} md={6}>
						<div style={{ display: 'flex', flexDirection: 'column' }}>
							...
							{tipoDestinatario !== 'USUARIOS' && (
								<Button variant="outlined" color="primary" onClick={handleMostrar}>
									Mostrar
								</Button>
							)}
						</div>
					...
					<Grid item xs={12} sm={12} md={6}>
						{destinatarios.length > 0 && <ListaDestinatarios rows={destinatarios} />}
					</Grid>
				</Grid>
			</Box>
		</div>
