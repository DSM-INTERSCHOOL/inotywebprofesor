...
import { postPublicacion, uploadAttachmentsPublicacion } from '../../services/publicaciones';
...
export const HorizontalLinearStepper = ({ tipoUsuario }) => {
	...
	function getStepContent(step) {
		switch (step) {
			case 0:
				return <TipoPublicacionContainer setErrorMessage={setErrorMessage} tipoUsuario={tipoUsuario} />;
			case 1:
	...
	const handleNext = async () => {
		try {
			...
			if (activeStep === 0 && tipoPublicacion === '') {
				...
				dispatch(setToastMessage('El Título de la Publicación no puede ser vacio'));
			} else if (activeStep === 3 && contenido.cuerpoPublicacion === '') {
				dispatch(setToastMessage('El Cuerpo de la Publicación no puede ser vacio'));
			} else if (
				activeStep === 5 &&
				(filtros.fechaInicialPublicacion === '' || filtros.fechaFinalPublicacion === '')
			) {
				dispatch(setToastMessage('La fecha Inicial/Final de la Publicación no pueden ser vacias'));
			} else {
				...
				if (activeStep === steps.length - 1) {
					...
					const destinatariosPublicacion = await getListaDestinatariosPublicacion('CELTA', destinatarios);
	...
	const uploadAttachments = async (idAccount, headers, tipoPublicacion, idPublicacion) => {
		try {
			await Promise.all(
		...
		<div className={classes.root}>
			...
			<div>
				...
				{activeStep === steps.length ? (
					<div>
						<Typography className={classes.instructions}>¡Publicación Enviada!</Typography>
